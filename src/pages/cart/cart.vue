...
import { isSelectedAllApi, changeCartItemNum, deleteCartItem, getCartListApi } from '@/api/cart'
...
const cartList = ref<Array<CartListResult>>([])
...
// 是否选中商品
async function onChangeSelected(goodItem: CartListResult) {
  goodItem.selected = !goodItem.selected
  await changeCartItemNum(goodItem.skuId, { selected: goodItem.selected })
}

// 商品数量改变
async function onChangeCount(ev: InputNumberBoxEvent) {
  await changeCartItemNum(ev.index, { count: ev.value })
}

// 删除商品
function onDeleteCart(skuId: string) {
  uni.showModal({
    content: '是否删除',
    confirmColor: '#27BA9B',
    success: async (res) => {
...
// 全选
async function onChangeSelectedAll() {
