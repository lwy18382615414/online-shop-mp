<template>
  <scroll-view enable-back-to-top scroll-y class="viewport">
    <view class="goods">
      ...
      <!-- 操作面板 -->
      <view class="action">
        <view @tap="openSku(1)" class="item arrow">
  ...
  <view v-if="goodDetail" class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="buttons">
      <view @tap="openSku(2)" class="addcart"> 加入购物车 </view>
      <view @tap="openSku(3)" class="payment"> 立即购买 </view>
    </view>
  </view>
...
function openSku(mode: number) {
  isShowSku.value = true
  modeValue.value = mode
}

async function addCart(ev: SkuPopupEvent) {
  const res = await addCartApi(ev._id, ev.buy_num)
  if (res.code === '1') {
    uni.showToast({
      icon: 'success',
      title: '添加成功',
